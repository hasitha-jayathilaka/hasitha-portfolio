...
export default function EnquiryModal({
  ...
  const errors = (() => {
    ...
    if (phone.trim() && !isValidPhone(phone)) e.phone = "Phone looks invalid (optional).";
    if (type === "Other" && otherType.trim().length < 3) e.otherType = "Please specify (min 3 characters).";
              ...
              {type === "Other" ? (
                <div>
                  ...
                  <input
                    ...
                    maxLength={80}
                  />
                  {errors.otherType ? <p className="mt-1 text-xs text-red-300">{errors.otherType}</p> : null}
                </div>
              ) : null}
